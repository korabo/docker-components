...
LABEL MAINTAINER="S.TAKEUCHI(KRB/SPG)" containerid="dvlvm-java_centos7"
ENV _ENVDEF_P_ ${_ENVDEF_}:${_ENVDEF_P_}
ENV _ENVDEF_ dvlvm-java_centos7
...
RUN set -xe && \
    mkdir ~/work && \
    cd ~/work && \
    wget https://ftp.jaist.ac.jp/pub/apache/maven/maven-3/${_MVN_}/binaries/apache-maven-${_MVN_}-bin.tar.gz && \
    tar -xzvf apache-maven-${_MVN_}-bin.tar.gz && \
    mv apache-maven-${_MVN_} /opt/ && \
    cd /opt && \
    ln -s /opt/apache-maven-${_MVN_} apache-maven && \
...
# Default User
USER $USER_GID:$USER_GNM
WORKDIR /home/$USER_GID
